...
function BaseTable<TData, TColumnId extends string = string>(props: TableProps<TData, TColumnId>, ref: React.Ref<TableHandle>) {
  ...
  const reactId = useId();
  const storageKey = storageKeyProp?.trim() ? storageKeyProp : `IstemUI-Table-${reactId}`;
  const scrollViewportRef = useRef<HTMLDivElement | null>(null);
  const autoMinWidthCacheRef = useRef<Map<TColumnId, number>>(new Map());

  const explicitMinWidthById = useMemo(() => new Map(columns.map((col) => [col.id as TColumnId, col.minWidth])), [columns]);
  ...
  const getColumnMinWidth = useCallback(
    (columnId: TColumnId): number | undefined => {
      ...
      const escapedColumnId = typeof CSS !== 'undefined' && typeof CSS.escape === 'function' ? CSS.escape(columnId) : columnId;
      const cells = scrollViewport.querySelectorAll<HTMLElement>(`[data-column-id="${escapedColumnId}"]`);
      ...
      for (const cell of cells) {
        measuredMinWidth = Math.max(measuredMinWidth, measureCellBreakSafeMinWidth(cell, probe));
      }
  ...
  const { sortedRows, sortState, toggleSort, resetSorting } = useTableSorting<TData, TColumnId>({
    data: rows,
    columns,
    storageKey,
    initialSortState,
  ...
  const renderHeaderCell = (column: TableColumn<TData, TColumnId>, columnIndex: number) => {
    const width = widths[column.id as TColumnId];
    const isSorted = sortState.columnId === column.id;
    const isLastColumn = columnIndex === columns.length - 1;

    const headerCellContext = { column, columnIndex };
    const headerCellSlotProps = resolveProps(slotProps?.headerCell, headerCellContext);
    const columnHeaderCellProps = resolveProps(column.slotProps?.headerCell, headerCellContext);
        ...
        <>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            {column.sortable ? (
              <TableSortLabel
                active={isSorted}
                direction={isSorted && sortState.direction !== 'none' ? sortState.direction : 'asc'}
    ...
    return slots?.headerCell ? (
      <React.Fragment key={column.id}>{slots.headerCell(propsForSlot, TableCell)}</React.Fragment>
    ) : (
      <TableCell key={column.id} {...propsForSlot} />
    );
  };

  const renderBodyCell = (column: TableColumn<TData, TColumnId>, row: TData, rowIndex: number, columnIndex: number) => {
    const width = widths[column.id as TColumnId];

    const bodyCellContext = { column, columnIndex, row, rowIndex };
    const bodyCellSlotProps = resolveProps(slotProps?.bodyCell, bodyCellContext);
    const columnBodyCellProps = resolveProps(column.slotProps?.bodyCell, bodyCellContext);
    ...
    return slots?.bodyCell ? (
      <React.Fragment key={column.id}>{slots.bodyCell(propsForSlot, TableCell)}</React.Fragment>
    ) : (
      <TableCell key={column.id} {...propsForSlot} />
    );
  };
  ...
  const tablePropsForSlot = {
    ...
    children: (
      <>
        <TableHead>
          {slots?.headerRow ? slots.headerRow(headerRowPropsForSlot, TableRow) : <TableRow {...headerRowPropsForSlot} />}
        </TableHead>

        <TableBody>
          {sortedRows.map((row, rowIndex) => {
            const bodyRowPropsForSlot = {
              ...resolveProps(slotProps?.bodyRow ?? slotProps?.bodyRowProps, { row, rowIndex }),
              children: columns.map((col, colIdx) => renderBodyCell(col, row, rowIndex, colIdx)),
            };

            return slots?.bodyRow ? (
              <React.Fragment key={rowIndex}>{slots.bodyRow(bodyRowPropsForSlot, TableRow)}</React.Fragment>
            ) : (
        ...
        {renderFooter && (
          <TableFooter>
            {slots?.footerRow ? slots.footerRow(footerPropsForSlot, TableRow) : <TableRow {...footerPropsForSlot} />}
          </TableFooter>
        )}
      </>
    ),
  ...
  const containerPropsForSlot = {
    ...
    children: (
      ...
      >
        ...
        >
          {slots?.table?.(tablePropsForSlot, TableMui) ?? <TableMui {...tablePropsForSlot} />}
        </div>
      </Paper>
    ),
  };
...
function measureCellBreakSafeMinWidth(cell: HTMLElement, probe: HTMLSpanElement): number {
  const computed = window.getComputedStyle(cell);
  const horizontalPadding = px(computed.paddingLeft) + px(computed.paddingRight);
  ...
  for (const token of tokens) {
    ...
    maxTokenWidth = Math.max(maxTokenWidth, Math.ceil(probe.getBoundingClientRect().width));
  }
...
function resolveSxToObject(theme: Theme, sx: SxProps<Theme> | undefined): object {
  ...
  if (!resolved) return {};
  if (Array.isArray(resolved)) return Object.assign({}, ...(resolved.filter(Boolean) as object[]));
...
function resolveResetButtonPosition(position: TableResetSpacingButtonPosition): React.CSSProperties {
  return {
    position: 'absolute',
    zIndex: 3,
...
const Table = forwardRef(BaseTable) as <TData, TColumnId extends string = string>(
